...
describe Travis::Addons::Plan::Task do
  ...
  let(:handler) { described_class.new({}, email_type: type, recipients: recipients, owner: owner, plan: plan) }
  ...
  shared_examples 'sends email' do |type|
    let(:type) { type }

    specify 'sends to filtered recipients' do
      mailer.expects(type).with([recipients.first], owner, plan).returns(email)
      handler.run
    end
  end

  shared_examples 'no email sent' do |type|
    let(:type) { type }

    specify do
      mailer.expects(type).never
      handler.run
    end
  end
